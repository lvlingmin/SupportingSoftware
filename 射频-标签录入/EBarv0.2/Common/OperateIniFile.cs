    ...
    public class OperateIniFile
    {
        ...
        public static DataTable ReadConfig(string iniFilePath)
        {
            ...
            try
            {
                ...
                if (dtIniInfo.Columns.Count < 1)  //给内存表中添加列，一个为配置文件的位置标志Pos，二是状态标志0或1
                {
                    dtIniInfo.Columns.Add("Pos", typeof(string));
                    dtIniInfo.Columns.Add("Value", typeof(string));
                }
                wfile = new FileStream(iniFilePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);//2018-06-20 zlx mod
                sr = new StreamReader(wfile);  //通过sr对象读配置文件
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    if (cLine.Length == 2 && dtIniInfo.Columns.Count == 2)  //读配置文件写入内存表（两列，1是位置，2是标志）
                    {
            ...
            {
                sr.Close();
                wfile.Close();//2018-06-20 zlx add
                wfile.Dispose();
            }
            //}
        }
        ...
        public static bool WriteConfigToFile(string section, string iniFilePath, DataTable dt)
        {
            ...
            FileStream wfile = new FileStream(iniFilePath, FileMode.Open, FileAccess.Write, FileShare.Read);
            StreamWriter sw = new StreamWriter(wfile);
            try
            {
                sw.WriteLine(section);
                for (int i = 0; i < dt.Rows.Count; i++)   //把配置文件循环写入
                {
