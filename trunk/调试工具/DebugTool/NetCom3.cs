...
namespace DebugTool
{
    class NetCom3
    {
        ...
        void KeepAlive(object obj)
        {
            while (true)
            {
                ...
                if (isConnect)
                {
                    keepaliveFlag = false;
                    Thread.Sleep(20000);
                    while (iapIsRun is true) //lyq iap 191130
                        Delay(1000);
        ...
        public void Send(String order, int orderType)
        {
            ...
            lock (this)
            {
                ...
                byte[] byteData = HexStringToByteArray(order);
                try
                {
            ...
            lock (locker)
            {
                ...
                try
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        MessageBox.Show("消息提示", "仪器连接已经断开，请检查网络连接状态，并重启系统！");
                    }
                    if (bytesRead > 0)
                    {
                        ...
                        try
                        {
                            state.sb.Append(ByteArrayToHexString(state.buffer));
                        }
                        catch (Exception e)
                        {
                        ...
                        foreach (string tempResponse in ReciveData)
                        {
                            ...
                            if (tempResponse.ToString().IndexOf("EB 90") > -1)
                            {
                                ...
                                if (orderTemp == "11 FF" || orderTemp == "11 AF" || orderTemp == "01 A0" || orderTemp == "11 A0" //“11 FF”版本号查询//仪器调教指令处理//仪器调试收到查询温度
    ...
    public class LogFile
    {
        ...
        public LogFile()
        {
            SW = new FileStream(Application.StartupPath + @"\NetLog\C" + DateTime.Now.ToString("yyyyMMdd HHmmss") + ".txt", FileMode.Create, FileAccess.Write, FileShare.ReadWrite, 100, FileOptions.Asynchronous);
